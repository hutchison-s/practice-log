...
function MultiLessonBlock({flex, students, onClick}: {flex: number, students: Enrollee[], onClick?: (student_id: string)=>void}) {
  ...
  return (
    <>
        <button className='w-full bg-black rounded border-[1px] border-teal-500 p-1' style={{flexGrow: `span ${flex}`}} onClick={toggleOpen}>
        ...
        <dialog ref={modalRef} className='modalStyle border-[1px] border-white/25 rounded-xl'>
            ...
            <div className="w-[300px] min-h-[400px] relative flex flex-col gap-2 px-4 py-8 relative">
                ...
                {students.map(s => <LessonBlock key={s.id} flex={Math.round(s.duration / 7.5)} student={s} onClick={onClick}/>)}
                <EmptyBlock flex={1} />
            </div>
        </dialog>
    </>
