...
function WeekSchedule({studentList}: {studentList: EnrolleeWithCurrentWeekPractice[]}) {
    ...
    <>
        <section className='w-full flex flex-wrap gap-2 md:gap-0 md:flex-nowrap md:h-[600px] border-[1px] border-white/25 border-l-transparent border-top-transparent rounded font-inter'>
            {days.map((dayTitle, index) => {
                ...
                return filtered.length != 0 && <ResponsiveDaySchedule key={index} dayTitle={dayTitle} students={filtered} onSelect={undefined}/>
            })}
        </section>
    </>
  )
