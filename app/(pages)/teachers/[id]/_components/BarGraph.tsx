    ...
    <div className='flex gap-1 w-full h-[140px] border-[1px] border-white/25 rounded p-1 items-end'>
        {data.map((each, index)=>(
            <div key={index} className='grid gap-1 flex-1'>
                <div style={{height: ((each / max) * 100)+'px'}} className='relative bg-gradient-to-br from-cyan-500 to-teal-800 w-full grid place-items-center rounded'>
                    <span className={((each / max) * 100) > 50 ? 'text-md text-shadow-xl text-white' : 'text-md text-shadow-xl text-white absolute left-1/2 bottom-full -translate-x-1/2'}>{data_labels[index]}</span>
                </div>
