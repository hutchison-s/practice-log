  ...
  useEffect(() => {
    ...
    const checkForMessages = async () => {
      const res = await fetch(
        `/api/students/${details?.student.id}/messages/unread`
      );
      if (res.ok) {
      ...
      <section className="w-full max-w-[600px] glass rounded-b-lg p-4 lg:rounded-r-lg lg:rounded-bl-none">
        <div className="flex gap-2 justify-between items-center mb-4">
          <h3 className="font-golos font-bold text-lg grid md:text-2xl">
            <span>{details.student.name}</span>
            <span className="text-sm font-inter font-light md:text-xl">
              {details.student.subject}
            </span>
          </h3>
          <div className="flex gap-2">
            <Link
              href={`/students/${details.student.id}/messages`}
            ...
            {details.time != 0 ? (
              <Link
                href={`/students/${details.student.id}/qr_code?code=${details.student.code}&time=${details.time}`}
                className="text-teal-500"
              >
                <QrCode
                  size={40}
