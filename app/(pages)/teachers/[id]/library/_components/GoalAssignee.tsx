...
function AssigneeDisplay({student, isAssigned}: {student: Enrollee, isAssigned: boolean}) {
  ...
  return (
    <label className='hover:bg-indigo-800 p-1 rounded my-1 flex justify-between'>
        ...
        <input type="checkbox" name={student.id+'-assignee'} value={student.id} id={student.id+'-assignee'} className='hidden' disabled={isAssigned} onChange={(e)=>setIsChecked(e.currentTarget.checked)} checked={isChecked}/>
