...
export default function LogInOutButton({isMobile, closeMenu}: {isMobile?: boolean, closeMenu?: ()=>void}) {
    ...
    return (
        isMobile
        ? user.id != ''
            ?   <MobileNavLink closeMenu={closeMenu ? closeMenu : () => null} icon={<LogOut size={80} aria-label="Sign Out"/>} label="Sign Out" url="/api/auth/logout" />
            :   <MobileNavLink closeMenu={closeMenu ? closeMenu : () => null} icon={<LogIn size={80} aria-label="Sign In"/>} label="Sign In" url="/login" />
        : user.id != ''
