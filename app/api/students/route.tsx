...
export async function POST(request: Request): apiResponse<Enrollee> {
    try {
        const {name, subject, teacher_id, weekly_goal, day_of_week, time_of_day, duration, timezone, group_id} = await request.json();
        if (!name || !subject || !teacher_id || !weekly_goal || !day_of_week || !time_of_day || !duration || !timezone) {
            console.error('missing required field(s): name, subject, teacher_id, weekly_goal, day_of_week, time_of_day, duration, timezone');
        }
        ...
        // }
        const insertResponse = await sql`INSERT INTO students (name, subject, teacher_id, weekly_goal, time_of_day, duration, day_of_week, timezone, group_id) VALUES (${name}, ${subject}, ${teacher_id}, ${weekly_goal}, ${time_of_day}, ${parseInt(duration)}, ${parseInt(day_of_week)}, ${timezone}, ${group_id || null}) RETURNING *`;
